...
// Funções existentes do aplicativo
function mostrarStatus(mensagem, erro=false){
  statusEl.textContent = mensagem;
  statusEl.className = erro ? 'status error' : 'status muted';
...
document.getElementById('btnSalvar').addEventListener('click',async()=>{
  ...
  const posicaoPreferida = posicaoPreferidaEl ? posicaoPreferidaEl.value.replace('preferida-','') : null;

  if(!nome||!pe||posicoes.length===0){
    mostrarStatus('Por favor, preencha todos os campos obrigatórios.',true); return;
  }

  const jogador = { nome, pe, papagaio, nivel, agilidade, posicoes, posicaoPreferida,
...
document.querySelectorAll('.posicoes-grid input[type="checkbox"]').forEach(el=>{
  el.addEventListener('change',()=>{
    ...
    if(isPref){
      document.querySelectorAll('.posicoes-grid input.preferida-check').forEach(other=>{ if(other!==el) other.checked = false; });
      const posId = el.id.replace('preferida-','');
      document.getElementById(posId).checked = el.checked;
    }
  });
});
...
function renderizarEquipes(){
  ...
  const html = Object.keys(equipes).map(key=>`
    <div class="team">
      <h4>Equipe ${key} <span class="muted small">(valor total: ${equipes[key].valor.toFixed(2)})</span></h4>
      <div class="team-list">
...
document.getElementById('btnDividir').addEventListener('click',()=>{
  ...
  jogadoresSelecionados.forEach((j,idx)=>{
    const equipeIdx = (idx % numEquipes) + 1;
    equipes[equipeIdx].jogadores.push(j);
    equipes[equipeIdx].valor += j.valorTotal;
  });
...
try{
  ...
  // Os listeners só serão ativados após o login
  onAuthStateChanged(auth, (user) => {
    if (user) {
...
}catch(e){ console.error('Firebase init',e); mostrarStatus('Erro ao conectar com Firebase: '+e.message,true) }